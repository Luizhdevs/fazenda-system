...
export function AdminRoute({ children }) {
  ...
  if (!usuario) return <Navigate to="/login" replace />
  if (!fazenda) return <Navigate to="/selecionar-fazenda" replace />
  if (fazenda.papel !== 'admin') return <Navigate to="/painel" replace />

  return children
}

export function SuperAdminRoute({ children }) {
  const { usuario, fazenda, superadmin, carregando } = useAuth()
  ...
  if (!usuario) return <Navigate to="/login" replace />
  if (!fazenda) return <Navigate to="/selecionar-fazenda" replace />
  if (!superadmin) return <Navigate to="/painel" replace />
